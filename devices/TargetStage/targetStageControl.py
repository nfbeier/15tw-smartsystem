...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.y_xps = XPS()
        self.z_xps = XPS()
        self.abs_max = [40.00000, 40.00000,40.00000]  # Maximum absolute position of the stage
        self.abs_min = [0.00000, 0.00000,0.00000]    # Minimum absolute position of the stage
        ...
        self.x_slider.valueChanged.connect(lambda: self.update_position_from_scrollbar("X_slider")) # Connect the scrollbar to the update function
        # Y Axis Slider
        ...
        self.y_slider.valueChanged.connect(lambda: self.update_position_from_scrollbar("Y_slider")) # Connect the scrollbar to the update function
        # Z Axis Slider
        ...
        self.z_slider.valueChanged.connect(lambda: self.update_position_from_scrollbar("Z_slider")) # Connect the scrollbar to the update function     
        ...
        self.ref = [0.00000, 0.00000,0.00000]  # Initialize reference point for both x and y axes
        ...
        self.return_both_btn.clicked.connect(lambda: self.ref_commands('return', 'both'))
             ...
             if cmd == 'set':
                 if axis == 'x':
                    ...
                    #  self.update_slider_marker()
                     ...
                     self.log_window.append(f"Clicked button! Set Ref. XYZ to ({self.ref[0] :.2f}, {self.ref[1] :.2f}, {self.ref[1] :.2f} ) mm.")
                     self.ref_x = round(self.x_xps.getStagePosition(self.x_axis),2)
                     self.ref_y = round(self.y_xps.getStagePosition(self.y_axis),2)
                     self.ref_z = round(self.z_xps.getStagePosition(self.z_axis),2)
                     self.ref = [self.ref_x,self.ref_y,self.ref_z]  # Update self.ref for three axes
    ...
    def raster_inp(self, inp):
            ...
            if inp == 'step_length':
                if self.step_length_line.text():
                    ...
                    self.log_window.append(f"Step Length changed  to: {self.step_length:.3f} mm")
                ...
                if self.sample_length_line.text():
                    ...
                    self.log_window.append(f"Sample Length changed to: {self.sample_length:.3f} mm")
                ...
                if self.sample_width_line.text():
                    ...
                    self.log_window.append(f"Sample Width changed to: {self.sample_width:.3f} mm")
                ...
                self.log_window.append(f"Set Bound X clicked. Sample Length: {self.sample_length:.3f}")
                ...
                self.log_window.append(f"Set Bound Y clicked. Sample Width: {self.sample_width:.3f}")  
            elif inp == 'Rep_rate':
                if self.rep_rate_line.text():
                    self.rep_rate = int(float(self.rep_rate_line.text()) * 1000)
                    self.log_window.append(f"Rep. Rate changed to: {self.rep_rate//1000:.1f} s")
            ...
            if self.step_length_line.text() and self.sample_length_line.text() and self.sample_width_line.text():
                ...
                if self.step_length > self.sample_length:
                    self.messages.setText('ERROR: Step length cannot be greater than X bound')
                    self.messages.setStyleSheet("color: red;font-size: 12pt")
                    self.error_flag = True
                    
                    return
                if self.step_length > self.sample_width:
                    self.messages.setText('ERROR: Step length cannot be greater than Y bound')
                    self.messages.setStyleSheet("color: red;font-size: 12pt")
                    self.error_flag = True
    ...
    def start_raster(self):
            ...
            self.rast_timer.setInterval(self.rep_rate)  # Set the interval to the desired rep rate
        ...
        if mve=='abs_x_length':
            if self.abs_x_line.text():
                if float(self.abs_x_line.text()) > self.abs_max[0]:
                    self.messages.setText('ERROR: Position X (abs.) is greater than the maximum value!')
        ...
        if mve=='abs_y_length':
            if self.abs_y_line.text():
                if float(self.abs_y_line.text()) > int(self.abs_max[1]):
                    self.messages.setText('ERROR: Position Y (abs.) is greater than the maximum value!')
                    self.messages.setStyleSheet("color: red;font-size: 11pt")
                    self.error_flag = True
                    return
                else:
        ...
        if mve=='abs_z_length':
            if self.abs_z_line.text():
                if float(self.abs_z_line.text()) > int(self.abs_max[1]):
                    self.messages.setText('ERROR: Position Z (abs.) is greater than the maximum value!')
                    self.messages.setStyleSheet("color: red;font-size: 11pt")
                    self.error_flag = True
                    return
                else:
    ...
    def absolute(self):
            ...
            '''
            abs = [round(self.x_xps.getStagePosition(self.x_axis),2), round(self.y_xps.getStagePosition(self.y_axis),2),round(self.z_xps.getStagePosition(self.z_axis),2)]
            if self.abs_x_line.text():     
                pos = float(self.abs_x_line.text())  
                self.log_window.append(f"X moved to: {float(self.abs_x_line.text()):.2f} mm") 
            else:
                pos =float(abs[0])
            self.x_xps.moveAbsolute(self.x_axis, pos)    
            if self.abs_y_line.text():
                pos = float(self.abs_y_line.text())
                self.log_window.append(f"Y moved to: {float(self.abs_y_line.text()):.2f} mm") 
            else:
                pos = float(abs[1])    
            self.y_xps.moveAbsolute(self.y_axis, pos)  
            if self.abs_z_line.text():
                pos = float(self.abs_z_line.text())
                self.log_window.append(f"Z moved to: {float(self.abs_z_line.text()):.2f} mm")
            else:
                pos = float(abs[2])
    ...
    def relative(self, btn):
        ...
        if btn == 'left' or btn == 'right':
            if not self.step_interval_x.text():
                self.messages.setText("ERROR: X Step Interval must be provided befor moving!")
        ...
        if btn == 'up' or btn == 'down':
            if not self.step_interval_y.text():
                self.messages.setText("ERROR: Y Step Interval must be provided befor moving!")
                self.messages.setStyleSheet("color:red;font-size:14px")
                return
            else:
                self.messages.clear()        
        if btn == 'up_z' or btn == 'down_z':
            if not self.step_interval_z.text():
                self.messages.setText("ERROR: Z Step Interval must be provided befor moving!")
                self.messages.setStyleSheet("color:red;font-size:14px")
                return
            else:
                self.messages.clear()   
        ...
        if self.step_interval_x.text():
            ...
            if btn == 'left':
                ...
                new_value = max(self.x_slider.minimum(), current_value[0] - int(dist * self.slider_factor))
                ...
                new_value = min(self.x_slider.maximum(), current_value[0] + int(dist * self.slider_factor))
                self.x_slider.setValue(new_value)                
        ...
        if self.step_interval_y.text():  
            ...
            if btn == 'up':
                ...
                new_value = max(self.y_slider.minimum(), current_value[1] -int(dist * self.slider_factor))
                ...
                new_value = min(self.y_slider.maximum(), current_value[1] +int(dist * self.slider_factor))                
                self.y_slider.setValue(new_value)
        if self.step_interval_z.text():  
            ...
            if btn == 'up_z':
                ...
                new_value = max(self.z_slider.minimum(), current_value[1] -int(dist * self.slider_factor))
                ...
                new_value = min(self.z_slider.maximum(), current_value[1] +int(dist * self.slider_factor))                
                self.z_slider.setValue(new_value)        
    ...
    def print_location(self):
            ...
            #  self.rel_lbl.setText(str(abs[0]-self.ref[0])+", "+str(abs[1]-self.ref[1]))
             ...
             self.position_x_ref.setText(f"Ref. X is: {self.ref[0]:.2f} mm"+",  "+f"X (rel.): {(abs[0]-self.ref[0]):.2f} mm")
             self.position_y_ref.setText(f"Ref. Y is: {self.ref[1]:.2f} mm"+",  "+f"Y (rel.): {(abs[1]-self.ref[1]):.2f} mm")
             self.position_z_ref.setText(f"Ref. Z is: {self.ref[2]:.2f} mm"+",  "+f"Z (rel.): {(abs[2]-self.ref[2]):.2f} mm")
            #  print(f"abs0:{abs[0]}")
             self.update_status(self.x_xps.getStageStatus(self.x_axis))   
